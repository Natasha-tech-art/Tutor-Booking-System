    ...
    >
      <div className="w-full max-w-md bg-white/10 backdrop-blur-2xl border border-white/20 rounded-[35px] p-8 shadow-2xl text-white mt-16">
        ...
        <form onSubmit={handleSignup} className="space-y-5">
          <div className="relative group">
            <input 
              ...
              required
              className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 px-4 focus:outline-none focus:border-blue-400 focus:bg-white/10 transition"
            />
          </div>
          ...
          <button 
            type="submit" 
            className="w-full bg-blue-600 hover:bg-blue-500 py-4 rounded-2xl font-bold text-lg mt-4 shadow-xl flex items-center justify-center gap-2 transition-all active:scale-95"
          >
        ...
        <p className="text-center mt-6 text-sm text-white/50">
          Already have an account? <Link to="/login" className="text-blue-400 cursor-pointer font-bold hover:underline">Log In</Link>
        </p>
      </div>
    </div>
  );
